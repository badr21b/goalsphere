...
export default function ThreeColumnLayout({
  ...
  const nextLeague = () =>
    setRightLeagueIndex((i) => (i + 1) % leagueSlugs.length);
  const prevLeague = () =>
    setRightLeagueIndex(
      (i) => (i - 1 + leagueSlugs.length) % leagueSlugs.length
    );
    ...
    const fetchAllLeagueData = async () => {
      try {
        ...
        for (const slug of Object.keys(LEAGUES) as LeagueSlug[]) {
          ...
          allConvertedMatches = [
            ...allConvertedMatches,
            ...convertedMatchesForLeague,
          ];
        }

        // Sort all matches by date/time
        allConvertedMatches.sort(
          (a, b) =>
  ...
  const scopedMatches = realMatches.filter(
    (m) => m.league === activeLeagueSlug
  );
  ...
  try {
    ...
    console.log("[ThreeColumnLayout] news snapshot", {
      ...
      sample: (filteredNews.length ? filteredNews : realNews)
        .slice(0, 5)
        .map((n) => ({
          id: n.id,
          title: n.title,
  ...
  useEffect(() => {
    ...
    const newsByCategory: Record<string, any[]> = {
      all: realNews,
      transfers: realNews.filter(
        (n) => /transfer/i.test(n.title) || /transfer/i.test(n.description)
      ),
      analysis: realNews.filter((n) =>
        /analysis|tactical|breakdown/i.test(n.title + " " + n.description)
      ),
      breaking: realNews.filter((n) => n.isBreaking),
      "premier-league": realNews.filter((n) => /premier/i.test(n.league.name)),
      "champions-league": realNews.filter((n) =>
        /champions/i.test(n.league.name)
      ),
      "saudi-pro-league": realNews.filter((n) => /saudi/i.test(n.league.name)),
      "la-liga": realNews.filter((n) =>
        /la liga|laliga|spa/i.test(n.league.name + " " + n.league.country)
      ),
      "serie-a": realNews.filter((n) =>
        /serie a|ita/i.test(n.league.name + " " + n.league.country)
      ),
      bundesliga: realNews.filter((n) =>
        /bundesliga|ger/i.test(n.league.name + " " + n.league.country)
      ),
    };
  ...
  const scrollMatchesUp = () => {
    if (groupedMatchesArray.length > 0) {
      const newIndex =
        currentMatchScrollIndex > 0
          ? currentMatchScrollIndex - 1
          : groupedMatchesArray.length - 1;
  ...
  const scrollMatchesDown = () => {
    if (groupedMatchesArray.length > 0) {
      const newIndex =
        currentMatchScrollIndex < groupedMatchesArray.length - 1
          ? currentMatchScrollIndex + 1
          : 0;
      setCurrentMatchScrollIndex(newIndex);
    }
  ...
  const uniqueMatches = scopedMatches.filter(
  ...
  const currentGroup = groupedMatchesArray[currentMatchScrollIndex] || [
    "No matches",
    [],
  ];
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col space-y-4 lg:hidden">
        ...
        >
          <h3 className="text-sm font-bold text-white mb-3 sm:mb-4">
            {t("home.latestNews")}
          </h3>
